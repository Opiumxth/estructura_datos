/**
 * nombreClase: BinarySearchTree.java
 * descripcion:
 *
 *
...
import paqListas.ListaDinamica;

import java.util.*;
import paqListas.Nodo;
...
public class BinarySearchTree<T extends Comparable<T>> {
    ...
    public NodoArbolBinario<T> add (NodoArbolBinario<T> origen, T elemento) {
        ...
        if (root == null) {
        	...
        	if (origen == null) { //el parametro pasado es invalido
        ...
        if (root == null) {
            ...
            while (!insertado) {
                ...
                if (aux.getElement().compareTo(elemento) > 0) {
                	...
                	// Paso a la derecho
                	// simtiene nodo derecho, se actualiza
    ...
    public int height(NodoArbolBinario<T> nodo) {
        ...
        if (nodo == null) {
            System.out.printf("Nodo nulo -> altura 0\n");
            return 0;
        }
        ...
        if (nodo.getLeft() != null) {
            leftHeight = height(nodo.getLeft());
        }

        if (nodo.getRight() != null) {
            rightHeight = height(nodo.getRight());
        }

        int h = 1 + Math.max(leftHeight, rightHeight);

        System.out.printf("Altura del nodo %s: %d\n",
                nodo.getElement().toString(),
                h);
    ...
    public int depth(NodoArbolBinario<T> nodo) {
        int depth = 0;
        if (nodo == null || nodo == root) return 0;
    ...
    public void getNodes(NodoArbolBinario<T> nodo, T elemento,
            ListaDinamica<NodoArbolBinario<T>> lista_nodos) {

        if (nodo.getElement().compareTo(elemento) == 0) {
            lista_nodos.anadir(nodo);
        }
    ...
    public ListaDinamica<T> getElements(NodoArbolBinario<T> nodo, T elemento) {
        ...
        Nodo< NodoArbolBinario<T> > actual = lista_nodos.getCabeza();

        while (actual != null) {
            elementos.anadir(actual.dato.getElement());
