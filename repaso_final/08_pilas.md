...
ACCION crearPila(PILA p)
   p.tope ← -1
FIN_ACCION

ACCION empilar(PILA p, TD dato)
   ENTERO n
   n ← p.tope + 1
   SI(n < MAX)
      p.pila[n] ← dato
...
ACCION empilar(NODO tope, TD dato)
   NODO nuevo
   nuevo ← new NODO
   nuevo.valor ← dato
   nuevo.sgte ← tope
...
ACCION depilar(NODO tope, TD dato)
   SI(tope = NULL)
      ESCRIBIR("Pila vacía")
   SINO
      NODO temp ← tope
...
**Enunciado:** Verificar si una expresión tiene paréntesis balanceados. Ej: `((a+b)*c)` → , `((a+b)` → No
...
int main() {
    cout << "((a+b)*c): " << verificarParentesis("((a+b)*c)") << endl;   // 1 OK
    cout << "((a+b): "    << verificarParentesis("((a+b)")    << endl;    // 0 X
    cout << "a+b): "      << verificarParentesis("a+b)")      << endl;    // 0 X
    return 0;
}
```
...
1. ¿Por qué el principio LIFO de las pilas es ideal para implementar algoritmos recursivos?
2. ¿Cuál es la diferencia fundamental entre una pila y una cola en términos de orden de acceso?
3. ¿Cómo se relaciona el concepto de pila con el manejo de memoria en la ejecución de funciones?
4. ¿Qué problema resuelve el uso de pilas para verificar paréntesis balanceados en expresiones?
5. ¿Cuándo conviene implementar una pila con arreglos y cuándo con listas enlazadas?
6. ¿Qué ocurriría si intentas hacer pop de una pila vacía en un programa real?
7. ¿Cómo se utiliza una pila para evaluar expresiones en notación postfija?
...
**Ejercicio 8.4 (Pseudocódigo → C/C++):** Use una pila para convertir un número decimal a binario. Primero resuelva en pseudocódigo, luego traduzca a C/C++.

**Ejercicio 8.5 (Tipo examen — Ambos):** Dada una pila de enteros, escriba una función que devuelva el valor mínimo SIN destruirla (use pila auxiliar). Primero en pseudocódigo, luego en C/C++.
